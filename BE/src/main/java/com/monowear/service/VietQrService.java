...
public class VietQrService {
    ...
    private static final Logger LOG = Logger.getLogger(VietQrService.class);
    private static final ObjectMapper MAPPER = new ObjectMapper();
    private static final HttpClient HTTP_CLIENT = HttpClient.newBuilder()
            .connectTimeout(Duration.ofSeconds(10))
            .build();

    @ConfigProperty(name = "vietqr.api-url")
    String apiUrl;

    @ConfigProperty(name = "vietqr.banks-url", defaultValue = "https://api.vietqr.io/v2/banks")
    String banksUrl;

    @ConfigProperty(name = "vietqr.lookup-url", defaultValue = "https://api.vietqr.io/v2/lookup")
    String lookupUrl;

    @ConfigProperty(name = "vietqr.lookup-enabled", defaultValue = "true")
    boolean lookupEnabled;

    @ConfigProperty(name = "vietqr.client-id")
    String clientId;

    @ConfigProperty(name = "vietqr.api-key")
    String apiKey;

    @ConfigProperty(name = "vietqr.account-no")
    String accountNo;

    @ConfigProperty(name = "vietqr.account-name")
    String accountName;

    @ConfigProperty(name = "vietqr.bank-code", defaultValue = "MB")
    String bankCode;

    @ConfigProperty(name = "vietqr.acq-id", defaultValue = "")
    String fallbackAcqId;

    @ConfigProperty(name = "vietqr.template")
    String template;
    ...
    public Map<String, String> generateQr(Long orderId, long amount) {
        ...
        try {
            ...
            LOG.infof(
                    "VietQR generated for Order #%d: bank=%s, bin=%s, account=%s, amount=%d",
    ...
    private String resolveAccountName(BankInfo bank) {
        if (!lookupEnabled || !hasApiCredentials()) {
        ...
        try {
            ...
            if (response.statusCode() >= 400 || !"00".equals(code) || resolvedName.isBlank()) {
                ...
                throw new BadRequestException("Tai khoan " + bank.displayName() + " "
                        + maskAccountNo(accountNo) + " khong hop le: " + message);
            }
            ...
            throw new BadRequestException("Ma BIN ngan hang VietQR khong hop le: " + bank.bin());
        } catch (Exception e) {
            throw new BadRequestException("Khong the xac thuc tai khoan VietQR: " + e.getMessage());
        }
    }

    private BankInfo resolveBankInfo() {
        String configuredBank = normalized(bankCode);
        if (configuredBank.isBlank()) {
            return fallbackBankInfo();
        }

        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(banksUrl))
            ...
            if (response.statusCode() >= 400 || !banks.isArray()) {
                String message = json.path("desc").asText("Khong the lay danh sach ngan hang VietQR");
                throw new BadRequestException("VietQR bank list error: " + message);
            }

            for (JsonNode bank : banks) {
                String code = bank.path("code").asText("");
                String shortName = bank.path("shortName").asText(bank.path("short_name").asText(""));
                String bin = bank.path("bin").asText("");

                if (matchesConfiguredBank(configuredBank, code, shortName, bin)) {
                    int transferSupported = bank.path("transferSupported").asInt(bank.path("isTransfer").asInt(1));
                    if (transferSupported != 1) {
                        throw new BadRequestException("Ngan hang " + displayName(code, shortName)
                                + " chua ho tro chuyen khoan VietQR");
                    }
                    ...
                    return new BankInfo(
                            ...
                            code,
                            shortName,
                            bank.path("name").asText(displayName(code, shortName))
                    );
                }
            }
            ...
            LOG.warnf("Cannot load VietQR bank list. Using configured fallback bin %s for bank %s",
                    fallback.bin(), fallback.displayName());
            return fallback;
        }
    }

    private BankInfo fallbackBankInfo() {
        if (fallbackAcqId == null || fallbackAcqId.isBlank()) {
            throw new BadRequestException("Thieu cau hinh VietQR bank-code/acq-id");
        }
    ...
    private Map<String, String> buildFallbackQr(
            Long orderId,
            long amount,
            String addInfo,
            BankInfo bank,
        ...
        String qrDataUrl = String.format(
    ...
    private Map<String, String> buildResult(
        ...
        result.put("accountNo", accountNo.trim());
        result.put("accountName", verifiedAccountName);
        result.put("bankBin", bank.bin());
        result.put("bankCode", bank.code());
        result.put("bankName", bank.displayName());
        result.put("acqId", bank.bin());
    ...
    private void validatePaymentInput(Long orderId, long amount) {
        ...
        if (isBlank(accountNo) || isBlank(accountName) || isBlank(template)) {
            throw new BadRequestException("Thieu cau hinh tai khoan nhan chuyen khoan VietQR");
        }
    }

    private boolean hasApiCredentials() {
        return !isBlank(clientId) && !clientId.contains("CLIENT_ID_HERE")
                && !isBlank(apiKey) && !apiKey.contains("API_KEY_HERE");
    }

    private boolean matchesConfiguredBank(String configuredBank, String code, String shortName, String bin) {
        return configuredBank.equals(normalized(code))
                || configuredBank.equals(normalized(shortName))
                || configuredBank.equals(normalized(bin));
    }

    private String normalized(String value) {
        return value == null ? "" : value.replaceAll("[\\s_-]", "").toUpperCase();
    }

    private String displayName(String code, String shortName) {
        if (!isBlank(shortName)) return shortName;
        if (!isBlank(code)) return code;
        return "VietQR Bank";
    }

    private String maskAccountNo(String value) {
    ...
    private record BankInfo(String bin, String code, String shortName, String name) {
        String displayName() {
